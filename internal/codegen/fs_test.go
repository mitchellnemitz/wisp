...
// cannot express. unwrap_or(env.get(...), fb)'s set-but-empty arm needs a
// per-run environment variable, which the golden harness does not provide per
// fixture, so it runs here via runNSDir's env plumbing.

// TestFS_EnvOr_SetButEmpty_ReturnsValue: a set-but-empty variable returns ""
// (present), NOT the fallback (spec 3.5), via unwrap_or(env.get(n), fb) --
// the replacement for the removed env_or sugar.
func TestFS_EnvOr_SetButEmpty_ReturnsValue(t *testing.T) {
	out, errb, code := runNSDir(t, `fn main() -> int {
  print("[${unwrap_or(env.get("WISP_EMPTY"), "FB")}]")
  return 0
